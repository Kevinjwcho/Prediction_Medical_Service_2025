...
### Defining new sup_demand function
first_doc = c(33651,	34422,	34955,	35556,	36475,	37438,	38770,	40347,	41845,	43576,	44781,	46312,	48584,	50285)
total_doc = c(82137,	84544,	86761,	90710,	92927,	95076,	97713,	100241,	102471,	105628,	107976,	109937,	112321,	114699)
first_prop = first_doc/total_doc

sup_demand_s1<- function(admin_set_1, admin_set_2, admin_set_3, demand_est_yr, mid_yr = NULL, reduce_rate, tech_rate = 0.005){
  med_admin_p_1 = c(2533, 3045, 3118, 3309, 3236, 2978, 3018, 3018, 3018) # 의대생: 20대로 가정 (2022년 보다 6년 전 데이터 (2016년) 부터 시작)
  med_admin_p_2 = c(1242, 218, 198, 198, 149, 129, 129, 80, 80) # 의전원: 30대로 가정
  med_admin_p_3 = c(100, 100, 100, 100, 100, 67, 85, 89, 85) # 편입생: 30세로 가정 -> 김영편입학원에서 데이터 가지고 옴 (의대 + 치의대)
  # admin_set_1 = 4977
  ...
  # new_med_total은 60대는 75% 그리고 70대 이상은 50% 정도만 일을 한다고 가정.
  result_sup = result_sup %>% mutate(new_med_total = age20_med + age30_med + age40_med + age50_med + age60_med*0.75 + age70_med*0.5)
  result_sup = result_sup %>% mutate(first_physician = case_when(year == 2025 ~ new_med_total*first_prop[length(first_prop)]*0.2,
                                                                 year == 2026 ~ new_med_total*first_prop[length(first_prop)]*0.3,
                                                                 year == 2027 ~ new_med_total*first_prop[length(first_prop)]*0.4,
                                                                 year == 2028 ~ new_med_total*first_prop[length(first_prop)]*0.5,
                                                                 year >= 2029 ~ new_med_total*first_prop[length(first_prop)]*0.6
  )
  )
  
  result <- demand_est_yr %>% left_join(result_sup) %>% dplyr::select(year, out_p_est, in_p_est, license_total, med_total, new_med_total, first_physician)
  result <- result %>% mutate(first_demand = case_when(year == 2025 ~ (out_p_est)*0.16,
                                                       year == 2026 ~ (out_p_est)*0.245,
                                                       year == 2027 ~ (out_p_est)*0.33,
                                                       year == 2028 ~ (out_p_est)*0.415,
                                                       year >= 2029 ~ (out_p_est)*0.5),
                              doc_capa = (out_p_est+in_p_est*3)/265/license_total,
                              doc_capa_med = (out_p_est+in_p_est*3)/265/new_med_total,
                              first_capa_med = first_demand/265/first_physician,
                              doc_demand_med = (out_p_est+in_p_est*3)/265/51,
                              doc_demand_med_tech = (out_p_est+in_p_est*3)/265/seq(51, by = 51*tech_rate, length.out = (2072 - 2022 + 1)),
                              sup_demand_med = new_med_total - doc_demand_med,
                              sup_demand_med_tech = new_med_total - doc_demand_med_tech,
                              doc_demand_med_tech_s1 = (out_p_est+in_p_est*3 - first_demand)/265/seq(51, by = 51*tech_rate, length.out = (2072 - 2022 + 1)),
                              sup_demand_med_tech_s1 = new_med_total- first_physician - doc_demand_med_tech_s1
  )
  return(result)
}
...
start_yr = 2025
result_sup_s1_0 <- sup_demand_s1(yr_candi[1], 80, 0, demand_est_yr, start_yr+5, 0.97, tech_rate = 0.0025)
result_sup_s1_250 <- sup_demand_s1(yr_candi[2], 80, 0, demand_est_yr, start_yr+5, 0.97, tech_rate = 0.0025)
result_sup_s1_500 <- sup_demand_s1(yr_candi[3], 80, 0, demand_est_yr, start_yr+5, 0.97, tech_rate = 0.0025)
result_sup_s1_750 <- sup_demand_s1(yr_candi[4], 80, 0, demand_est_yr, start_yr+5, 0.97, tech_rate = 0.0025)
result_sup_s1_1000 <- sup_demand_s1(yr_candi[5], 80, 0, demand_est_yr, start_yr+5, 0.97, tech_rate = 0.0025)

## colnames changing
colnames(result_sup_s1_0)[-c(1:3)] <-paste0(colnames(result_sup_s1_0)[-c(1:3)], "_0" )
colnames(result_sup_s1_250)[-c(1:3)] <-paste0(colnames(result_sup_s1_250)[-c(1:3)], "_250" )
colnames(result_sup_s1_500)[-c(1:3)] <-paste0(colnames(result_sup_s1_500)[-c(1:3)], "_500" )
colnames(result_sup_s1_750)[-c(1:3)] <-paste0(colnames(result_sup_s1_750)[-c(1:3)], "_750" )
colnames(result_sup_s1_1000)[-c(1:3)] <-paste0(colnames(result_sup_s1_1000)[-c(1:3)], "_1000" )

result_s1_5y <- result_sup_s1_0 %>% left_join(result_sup_s1_250, by = "year") %>%
  left_join(result_sup_s1_500, by = "year") %>% 
  left_join(result_sup_s1_750, by = "year") %>%
  left_join(result_sup_s1_1000, by = "year")

result_sup_s1_0 <- sup_demand_s1(yr_candi[1], 80, 0, demand_est_yr, start_yr+10, 0.97, tech_rate = 0.0025)
result_sup_s1_250 <- sup_demand_s1(yr_candi[2], 80, 0, demand_est_yr, start_yr+10, 0.97, tech_rate = 0.0025)
result_sup_s1_500 <- sup_demand_s1(yr_candi[3], 80, 0, demand_est_yr, start_yr+10, 0.97, tech_rate = 0.0025)
result_sup_s1_750 <- sup_demand_s1(yr_candi[4], 80, 0, demand_est_yr, start_yr+10, 0.97, tech_rate = 0.0025)
result_sup_s1_1000 <- sup_demand_s1(yr_candi[5], 80, 0, demand_est_yr, start_yr+10, 0.97, tech_rate = 0.0025)
...
result_s1_10y <- result_sup_s1_0 %>% left_join(result_sup_s1_250, by = "year") %>%
  left_join(result_sup_s1_500, by = "year") %>% 
  left_join(result_sup_s1_750, by = "year") %>%
  left_join(result_sup_s1_1000, by = "year")

result_sup_s1_0 <- sup_demand_s1(yr_candi[1], 80, 0, demand_est_yr, start_yr+15, 0.97, tech_rate = 0.0025)
result_sup_s1_250 <- sup_demand_s1(yr_candi[2], 80, 0, demand_est_yr, start_yr+15, 0.97, tech_rate = 0.0025)
result_sup_s1_500 <- sup_demand_s1(yr_candi[3], 80, 0, demand_est_yr, start_yr+15, 0.97, tech_rate = 0.0025)
result_sup_s1_750 <- sup_demand_s1(yr_candi[4], 80, 0, demand_est_yr, start_yr+15, 0.97, tech_rate = 0.0025)
result_sup_s1_1000 <- sup_demand_s1(yr_candi[5], 80, 0, demand_est_yr, start_yr+15, 0.97, tech_rate = 0.0025)
...
result_s1_15y <- result_sup_s1_0 %>% left_join(result_sup_s1_250, by = "year") %>%
  left_join(result_sup_s1_500, by = "year") %>% 
  left_join(result_sup_s1_750, by = "year") %>%
  left_join(result_sup_s1_1000, by = "year")

p <- ggplot(result_s1_5y, aes(x = year))+geom_line(aes(y = sup_demand_med_tech_s1_0, col = "+0 (3018)")) +
  geom_line(aes(y = sup_demand_med_tech_s1_250, col = "+250 (3268)")) + 
  geom_line(aes(y = sup_demand_med_tech_s1_500, col = "+500 (3518)")) +
  geom_line(aes(y = sup_demand_med_tech_s1_750, col = "+750 (3768)")) +
  geom_line(aes(y = sup_demand_med_tech_s1_1000, col = "+1000 (4018)")) +
  ylab("") + 
  scale_color_manual(
...
ggsave("sup_dem_5yr_025tech_s1.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)

result_s1_5y$year[which(result_s1_5y$sup_demand_med_tech_s1_0 < 0)]
result_s1_5y$sup_demand_med_tech_s1_0[which.min(result_s1_5y$sup_demand_med_tech_s1_0)]
result_s1_5y$year[which.min(result_s1_5y$sup_demand_med_tech_s1_0)]

p <- ggplot(result_s1_10y, aes(x = year))+geom_line(aes(y = sup_demand_med_tech_s1_0, col = "+0 (3018)")) +
  geom_line(aes(y = sup_demand_med_tech_s1_250, col = "+250 (3268)")) + 
  geom_line(aes(y = sup_demand_med_tech_s1_500, col = "+500 (3518)")) +
  geom_line(aes(y = sup_demand_med_tech_s1_750, col = "+750 (3768)")) +
  geom_line(aes(y = sup_demand_med_tech_s1_1000, col = "+1000 (4018)")) +
...
ggsave("sup_dem_10yr_025tech_s1.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)

result_s1_10y$year[which(result_s1_10y$sup_demand_med_tech_s1_0 < 0)]
result_s1_10y$sup_demand_med_tech_s1_0[which.min(result_s1_10y$sup_demand_med_tech_s1_0)]
result_s1_10y$year[which.min(result_s1_10y$sup_demand_med_tech_s1_0)]

p <- ggplot(result_s1_15y, aes(x = year))+geom_line(aes(y = sup_demand_med_tech_s1_0, col = "+0 (3018)")) +
  geom_line(aes(y = sup_demand_med_tech_s1_250, col = "+250 (3268)")) + 
  geom_line(aes(y = sup_demand_med_tech_s1_500, col = "+500 (3518)")) +
  geom_line(aes(y = sup_demand_med_tech_s1_750, col = "+750 (3768)")) +
  geom_line(aes(y = sup_demand_med_tech_s1_1000, col = "+1000 (4018)")) +
...
ggsave("sup_dem_15yr_025tech_s1.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)

result_s1_15y$year[which(result_s1_15y$sup_demand_med_tech_s1_0 < 0)]
result_s1_15y$sup_demand_med_tech_s1_0[which.min(result_s1_15y$sup_demand_med_tech_s1_0)]
result_s1_15y$year[which.min(result_s1_15y$sup_demand_med_tech_s1_0)]


# S1: tech_rate 0.5% ------------------------------------------------------

start_yr = 2025
result_sup_s1_0 <- sup_demand_s1(yr_candi[1], 80, 0, demand_est_yr, start_yr+5, 0.97)
result_sup_s1_250 <- sup_demand_s1(yr_candi[2], 80, 0, demand_est_yr, start_yr+5, 0.97)
result_sup_s1_500 <- sup_demand_s1(yr_candi[3], 80, 0, demand_est_yr, start_yr+5, 0.97)
result_sup_s1_750 <- sup_demand_s1(yr_candi[4], 80, 0, demand_est_yr, start_yr+5, 0.97)
result_sup_s1_1000 <- sup_demand_s1(yr_candi[5], 80, 0, demand_est_yr, start_yr+5, 0.97)
...
result_sup_s1_0 <- sup_demand_s1(yr_candi[1], 80, 0, demand_est_yr, start_yr+10, 0.97)
result_sup_s1_250 <- sup_demand_s1(yr_candi[2], 80, 0, demand_est_yr, start_yr+10, 0.97)
result_sup_s1_500 <- sup_demand_s1(yr_candi[3], 80, 0, demand_est_yr, start_yr+10, 0.97)
result_sup_s1_750 <- sup_demand_s1(yr_candi[4], 80, 0, demand_est_yr, start_yr+10, 0.97)
result_sup_s1_1000 <- sup_demand_s1(yr_candi[5], 80, 0, demand_est_yr, start_yr+10, 0.97)
...
result_sup_s1_0 <- sup_demand_s1(yr_candi[1], 80, 0, demand_est_yr, start_yr+15, 0.97)
result_sup_s1_250 <- sup_demand_s1(yr_candi[2], 80, 0, demand_est_yr, start_yr+15, 0.97)
result_sup_s1_500 <- sup_demand_s1(yr_candi[3], 80, 0, demand_est_yr, start_yr+15, 0.97)
result_sup_s1_750 <- sup_demand_s1(yr_candi[4], 80, 0, demand_est_yr, start_yr+15, 0.97)
result_sup_s1_1000 <- sup_demand_s1(yr_candi[5], 80, 0, demand_est_yr, start_yr+15, 0.97)
...
ggsave("sup_dem_5yr_05tech_s1.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)
...
ggsave("sup_dem_10yr_05tech_s1.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)
...
ggsave("sup_dem_15yr_05tech_s1.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)
