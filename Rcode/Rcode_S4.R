...
if(Sys.info()[["sysname"]] == "Linux"){
  path <- c("/mnt/c/Users/kevin/Dropbox/Desktop/Prediction_medical_service/data/pop_est_sido.xlsx")
}else{
  path <- c("~/Dropbox/Desktop/Prediction_medical_service/data/pop_est_sido.xlsx")
}

pop_est_sido <- readxl::read_xlsx(path)
# str(pop_est_sido)
colnames(pop_est_sido)[1:5] <- c("scen", "sido", "sex", "age", "etc")
pop_est_sido <- pop_est_sido %>% dplyr::select(-scen, -etc) %>% melt(id.vars = c("sido", "sex", "age"))
# head(pop_est_sido)

pop_est_sido <- pop_est_sido %>% mutate(sido = case_when(sido == "서울특별시" ~ "Seoul",
                                                         sido == "부산광역시" ~ "Busan",
                                                         sido == "대구광역시" ~ "Daegu",
                                                         sido == "인천광역시" ~ "Incheon",
                                                         sido == "광주광역시" ~ "Gwangju",
                                        ...
                                        year = gsub(" 년", "", variable) %>% as.numeric,
)


demand_22yr_fin # 2022 yr demand estimation by age and sex per individual

pop_est_sido <- pop_est_sido %>% left_join(demand_22yr_fin, by = c("sex", "age"))

sido_demand <- pop_est_sido %>% mutate(out_p_est = value*out_per,
                                       in_p_est = value*in_per) %>%
  group_by(sido, year) %>% summarise(out_p_est = sum(out_p_est),
                                     in_p_est = sum(in_p_est))

# table(pop_est_sido$sido)
# table(pop_est_sido$sex)
# table(pop_est_sido$age)

### read doctor data ----
if(Sys.info()[["sysname"]] == "Linux"){
  path <- c("/mnt/c/Users/kevin/Dropbox/Desktop/Prediction_medical_service/data/sido_doc.xlsx")
}else{
  path <- c("~/Dropbox/Desktop/Prediction_medical_service/data/sido_doc.xlsx")
}

doc_sido <- readxl::read_xlsx(path)

doc_sido <- doc_sido %>% dplyr::filter(sido2 == "소계") %>% dplyr::select(-sido2) %>% 
  mutate(sido = case_when(sido1 == "서울" ~ "Seoul",
                          sido1 == "부산" ~ "Busan",
                          sido1 == "대구" ~ "Daegu",
                          sido1 == "인천" ~ "Incheon",
                          sido1 == "광주" ~ "Gwangju",
                          ...
                          sido1 == "경북" ~ "Kyungbuk",
                          sido1 == "경남" ~ "Kyungnam",
                          sido1 == "제주" ~ "Jaeju") ) %>% dplyr::select(-sido1) %>% 
  mutate(prop_doc = num_doc / sum(num_doc))


# History of proportion by sido -------------------------------------------

### read doctor data ----
if(Sys.info()[["sysname"]] == "Linux"){
  path <- c("/mnt/c/Users/kevin/Dropbox/Desktop/Prediction_medical_service/data/sido_doc_2010-2022.xlsx")
}else{
  path <- c("~/Dropbox/Desktop/Prediction_medical_service/data/sido_doc_2010-2022.xlsx")
}


doc_sido <- lapply(1:13, function(x) readxl::read_xlsx(path, sheet = x))
doc_sido_1 = lapply(1:length(doc_sido), function(x){
  y = doc_sido[[x]][, c(1:3)]
  colnames(y) = c("sido1", "sido2", "num_doc")
  y = y %>% dplyr::filter(sido2 == "소계") %>% dplyr::select(-sido2) %>% 
    mutate(sido = case_when(sido1 == "서울" ~ "Seoul",
                            sido1 == "부산" ~ "Busan",
                            sido1 == "대구" ~ "Daegu",
                            sido1 == "인천" ~ "Incheon",
                            ...
                            sido1 == "경남" ~ "Kyungnam",
                            sido1 == "제주" ~ "Jaeju"),
           num_doc = as.numeric(num_doc),
           year = 2009+x,
           category = case_when(sido %in% c("Seoul", "Gyunggi") ~ "Capital",
                                sido %in% c("Busan", "Daegu", "Incheon", "Gwangju", "Daejeon", "Ulsan", "Sejong") ~ "Metro",
                                sido %in% c("Gwangwon", "Chungbuk", "Chungnam", "Jeonbuk", "Jeonnam", "Kyungbuk", "Kyungnam", "Jaeju") ~ "Rural"),
           capital = case_when(sido %in% c("Seoul", "Gyunggi", "Incheon") ~ "CR",
                               TRUE ~ "NCR")
    ) %>% dplyr::select(-sido1) %>% 
    mutate(prop_doc_sido = num_doc / sum(num_doc))
  return(y)
})

doc_sido_2 = do.call(rbind, doc_sido_1)

p = ggplot(doc_sido_2 %>% filter(category == "Capital"), aes(x = year, y = prop_doc_sido, color = sido))+
  geom_line() + ggtitle("Capital")
ggsave("prop_doc_sido_capital.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)

p = ggplot(doc_sido_2 %>% filter(category == "Metro"), aes(x = year, y = prop_doc_sido, color = sido))+
  geom_line()+ ggtitle("Metro")
ggsave("prop_doc_sido_metro.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)

p = ggplot(doc_sido_2 %>% filter(category == "Rural"), aes(x = year, y = prop_doc_sido, color = sido))+
  geom_line()+ ggtitle("Rural")
ggsave("prop_doc_sido_rural.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)


doc_sido_capital = doc_sido_2 %>% group_by(year, capital) %>% summarise(sum_doc_CR = sum(num_doc))
doc_sido_sum = doc_sido_2 %>% group_by(year) %>% summarise(sum_doc = sum(num_doc))
doc_sido_capital = doc_sido_capital %>% left_join(doc_sido_sum, by = "year") %>% mutate(prop_doc_CR = sum_doc_CR / sum_doc)

p = ggplot(doc_sido_capital, aes(x = year, y = prop_doc_CR, color = capital))+
  geom_line() + ggtitle("Capital Region vs Non-Capital Region")
ggsave("prop_doc_CR.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)

## regression analysis
fit_CR = lm(prop_doc_CR ~ year, data = doc_sido_capital %>% filter(capital == "CR"))
summary(fit_CR)

fit_NCR = lm(prop_doc_CR ~ year, data = doc_sido_capital %>% filter(capital == "NCR"))
summary(fit_NCR)

doc_sido_capital_22 = doc_sido_capital %>% filter(year == 2022)

# Define the difference btw supply and demand by CR and NCR ----------------

sup_demand_CR <- function(admin_set_1, admin_set_2, admin_set_3, sido_demand, doc_sido_capital_22, mid_yr = NULL, reduce_rate, tech_rate = 0.005, prop_fix = F){
  med_admin_p_1 = c(2533, 3045, 3118, 3309, 3236, 2978, 3018, 3018, 3018) # 의대생: 20대로 가정 (2022년 보다 6년 전 데이터 (2016년) 부터 시작)
  med_admin_p_2 = c(1242, 218, 198, 198, 149, 129, 129, 80, 80) # 의전원: 30대로 가정
  med_admin_p_3 = c(100, 100, 100, 100, 100, 67, 85, 89, 85) # 편입생: 30세로 가정 -> 김영편입학원에서 데이터 가지고 옴 (의대 + 치의대)
  dead_rate = c(35.5, 56.5, 114.7, 248.6, 547.4, 1491.4) # 2022 사망률
  ...
  age_init = age_med_doc_2022 + age_nonmed_doc_2022 # 2022년 의사수
  age_med_prop = age_med_doc_2022/age_init # 2022년 의료기관 의사 비율
  result_sup<- supply_pred(year = 2052, med_admin_p_1, med_admin_p_2, med_admin_p_3,
                           admin_set_1, admin_set_2, admin_set_3, dead_rate, pass_rate,
                           age_init, age_med_prop, mid_yr, reduce_rate)
  ...
  sup_capital = expand.grid(year = 2022:2052, capital = doc_sido_capital_22$capital) %>% left_join(doc_sido_capital_22 %>% ungroup %>% dplyr::select(-year), by = c("capital"))
  if(prop_fix == F){
    result_sup_capital = sup_capital %>% left_join(result_sup, by = "year") %>% mutate(new_med_total_CR = case_when(capital == "CR" ~ (prop_doc_CR+(year-2022)*0.002)*new_med_total,
                                                                                                                    TRUE ~ (prop_doc_CR+(year-2022)*-0.002)*new_med_total))  
  }else{
    result_sup_capital = sup_capital %>% left_join(result_sup, by = "year") %>% mutate(new_med_total_CR = case_when(capital == "CR" ~ (prop_doc_CR)*new_med_total,
                                                                                                                    TRUE ~ (prop_doc_CR)*new_med_total))
  }
  
  capital_demand = sido_demand %>% mutate(capital = case_when(sido %in% c("Seoul", "Gyunggi", "Incheon") ~ "CR",
                                                              TRUE ~ "NCR")) %>% group_by(capital, year) %>% summarise(out_p_est = sum(out_p_est),
                                                                                                                       in_p_est = sum(in_p_est))
  
  result <- capital_demand %>% left_join(result_sup_capital, by = c("year", "capital")) %>% dplyr::select(capital, year, out_p_est, in_p_est, license_total, med_total, new_med_total_CR)
  tech_load = seq(51, by = 51*tech_rate, length.out = (2052 - 2022 + 1))
  tech_load = rep(tech_load, 2)
  result$tech = tech_load
  result <- result %>% mutate(doc_capa_med = (out_p_est+in_p_est*3)/265/new_med_total_CR,
                              doc_demand_med = (out_p_est+in_p_est*3)/265/51,
                              doc_demand_med_tech = (out_p_est+in_p_est*3)/265/tech,
                              sup_demand_med = new_med_total_CR - doc_demand_med,
                              sup_demand_med_tech = new_med_total_CR - doc_demand_med_tech
  )
  return(result)
}

## visualization: Without policy duration 0%
start_yr = 2025
result_0_CR <- sup_demand_CR(yr_candi[1], 80, 0, sido_demand, doc_sido_capital_22, mid_yr = NULL, reduce_rate = 1)
result_250_CR <- sup_demand_CR(yr_candi[2], 80, 0, sido_demand, doc_sido_capital_22, mid_yr = NULL, reduce_rate = 1)
result_500_CR <- sup_demand_CR(yr_candi[3], 80, 0, sido_demand, doc_sido_capital_22, mid_yr = NULL, reduce_rate = 1)

## colnames changing
colnames(result_0_CR)[-c(1:2)] <-paste0(colnames(result_0_CR)[-c(1:2)], "_0" )
colnames(result_250_CR)[-c(1:2)] <-paste0(colnames(result_250_CR)[-c(1:2)], "_250" )
colnames(result_500_CR)[-c(1:2)] <-paste0(colnames(result_500_CR)[-c(1:2)], "_500" )

result_s4_A1 <- result_0_CR %>% left_join(result_250_CR, by = c("capital", "year")) %>%
  left_join(result_500_CR, by = c("capital", "year"))

# Define gradient colors for each capital group
gradient_colors_capital_CR <- c("lightblue", "skyblue", "dodgerblue")
gradient_colors_capital_NCR <- c("lightgreen", "seagreen", "darkgreen")

# Define color palettes for each group
color_palette <- c(
  setNames(colorRampPalette(gradient_colors_capital_CR)(3), 
           c("+0 (CR)", "+250 (CR)", "+500 (CR)")),
  setNames(colorRampPalette(gradient_colors_capital_NCR)(3), 
           c("+0 (NCR)", "+250 (NCR)", "+500 (NCR)"))
)

# Plot with corrected color aesthetic
p <- ggplot(result_s4_A1, aes(x = year)) +
  geom_line(aes(
    y = sup_demand_med_0, 
    color = paste("+0 (", capital, ")", sep = ""),
    group = interaction(capital, "+0")
  )) +
  geom_line(aes(
    y = sup_demand_med_250, 
    color = paste("+250 (", capital, ")", sep = ""),
    group = interaction(capital, "+250")
  )) + 
  geom_line(aes(
    y = sup_demand_med_500, 
    color = paste("+500 (", capital, ")", sep = ""),
    group = interaction(capital, "+500")
  )) +
  ylab("") + 
  scale_color_manual(
    values = color_palette,
    breaks = names(color_palette)  # Ensure these match the `color` aesthetic
  ) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "darkgray") +
  labs(color = "Supply Level") +  # Custom legend title 
  ggtitle("A1: 0% tech growth rate") + 
  ylim(c(-25000, 20000))
ggsave("s4_A1_0.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)


# Plot with corrected color aesthetic
p <- ggplot(result_s4_A1, aes(x = year)) +
  geom_line(aes(
    y = sup_demand_med_tech_0, 
    color = paste("+0 (", capital, ")", sep = ""),
    group = interaction(capital, "+0")
  )) +
  geom_line(aes(
    y = sup_demand_med_tech_250, 
    color = paste("+250 (", capital, ")", sep = ""),
    group = interaction(capital, "+250")
  )) + 
  geom_line(aes(
    y = sup_demand_med_tech_500, 
    color = paste("+500 (", capital, ")", sep = ""),
    group = interaction(capital, "+500")
  )) +
  ylab("") + 
  ...
  geom_hline(yintercept = 0, linetype = "dashed", color = "darkgray") +
  labs(color = "Supply Level") +  # Custom legend title 
  ggtitle("A1: 0.5% tech growth rate") + 
  ylim(c(-25000, 20000))
ggsave("s4_A1_0.5.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)


## visualization: Without policy duration ideal A2

start_yr = 2025
result_0_CR <- sup_demand_CR(yr_candi[1], 80, 0, sido_demand, doc_sido_capital_22, mid_yr = NULL, reduce_rate = 1, prop_fix = T)
result_250_CR <- sup_demand_CR(yr_candi[2], 80, 0, sido_demand, doc_sido_capital_22, mid_yr = NULL, reduce_rate = 1, prop_fix = T)
result_500_CR <- sup_demand_CR(yr_candi[3], 80, 0, sido_demand, doc_sido_capital_22, mid_yr = NULL, reduce_rate = 1, prop_fix = T)
...
result_s4_A2 <- result_0_CR %>% left_join(result_250_CR, by = c("capital", "year")) %>%
  left_join(result_500_CR, by = c("capital", "year"))


# Plot with corrected color aesthetic
p <- ggplot(result_s4_A2, aes(x = year)) +
  geom_line(aes(
    y = sup_demand_med_0, 
    color = paste("+0 (", capital, ")", sep = ""),
    group = interaction(capital, "+0")
  ...
  geom_hline(yintercept = 0, linetype = "dashed", color = "darkgray") +
  labs(color = "Supply Level") +  # Custom legend title 
  ggtitle("A2: 0% tech growth rate") + 
  ylim(c(-25000, 20000))
ggsave("s4_A2_0.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)
...
p <- ggplot(result_s4_A2, aes(x = year)) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed", color = "darkgray") +
  labs(color = "Supply Level") +  # Custom legend title 
  ggtitle("A2: 0.5% tech growth rate") + 
  ylim(c(-25000, 20000))
ggsave("s4_A2_0.5.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)


## visualization: With policy duration
duration_year = c(5, 10, 15)

for(j in duration_year){
  start_yr = 2025
  result_0_CR <- sup_demand_CR(yr_candi[1], 80, 0, sido_demand, doc_sido_capital_22, mid_yr = start_yr+j, reduce_rate = 0.97)
  result_250_CR <- sup_demand_CR(yr_candi[2], 80, 0, sido_demand, doc_sido_capital_22, mid_yr = start_yr+j, reduce_rate = 0.97)
  result_500_CR <- sup_demand_CR(yr_candi[3], 80, 0, sido_demand, doc_sido_capital_22, mid_yr = start_yr+j, reduce_rate = 0.97)
  ...
  p <- ggplot(result_s4_A1, aes(x = year)) +
    ...
    geom_hline(yintercept = 0, linetype = "dashed", color = "darkgray") +
    labs(color = "Supply Level") +  # Custom legend title 
    ggtitle(paste0("A1: 0% tech growth rate, policy ", j, " years")) + 
    ylim(c(-25000, 20000))
  plot_n = paste0("s4_A1_0_", j, "yr.png")
  ggsave(plot_n, plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)
  ...
  p <- ggplot(result_s4_A1, aes(x = year)) +
    ...
    geom_hline(yintercept = 0, linetype = "dashed", color = "darkgray") +
    labs(color = "Supply Level") +  # Custom legend title 
    ggtitle(paste0("A1: 0.5% tech growth rate, policy ", j, " years")) + 
    ylim(c(-25000, 20000))
  plot_n = paste0("s4_A1_0.5_", j, "yr.png")
  ggsave(plot_n, plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)
  ...
  start_yr = 2025
  result_0_CR <- sup_demand_CR(yr_candi[1], 80, 0, sido_demand, doc_sido_capital_22, mid_yr = start_yr+j, reduce_rate = 0.97, prop_fix = T)
  result_250_CR <- sup_demand_CR(yr_candi[2], 80, 0, sido_demand, doc_sido_capital_22, mid_yr = start_yr+j, reduce_rate = 0.97, prop_fix = T)
  result_500_CR <- sup_demand_CR(yr_candi[3], 80, 0, sido_demand, doc_sido_capital_22, mid_yr = start_yr+j, reduce_rate = 0.97, prop_fix = T)
  ...
  p <- ggplot(result_s4_A2, aes(x = year)) +
    ...
    geom_hline(yintercept = 0, linetype = "dashed", color = "darkgray") +
    labs(color = "Supply Level") +  # Custom legend title 
    ggtitle(paste0("A2: 0% tech growth rate, policy ", j, " years")) + 
    ylim(c(-25000, 20000))
  plot_n = paste0("s4_A2_0_", j, "yr.png")
  ggsave(plot_n, plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)
  ...
  p <- ggplot(result_s4_A2, aes(x = year)) +
    ...
    geom_hline(yintercept = 0, linetype = "dashed", color = "darkgray") +
    labs(color = "Supply Level") +  # Custom legend title 
    ggtitle(paste0("A2: 0.5% tech growth rate, policy ", j, " years")) + 
    ylim(c(-25000, 20000))
  plot_n = paste0("s4_A2_0.5_", j, "yr.png")
  ggsave(plot_n, plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)
