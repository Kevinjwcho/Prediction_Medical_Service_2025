...
ggplot(demand_est_yr1, aes(x = year, y = out_p_est))+geom_line() + ylim(c(0, 1.2*10^9)) + ggtitle("Prediction of outpatient visits")
demand_est_yr[which.max(demand_est_yr1$out_p_est),]
p = ggplot(demand_est_yr_age, aes(x = year, y = out_p_est, fill = age)) + geom_area(position = 'stack') + ggtitle("Prediction of outpatient visits")
ggsave("demand_inp_age.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)
# ggplot(demand_est_yr_age, aes(x = year, y = out_p_est, fill = age)) + geom_area(position = 'fill') + ggtitle("Prediction of outpatient visits")

ggplot(demand_est_yr1, aes(x = year, y = in_p_est))+geom_line() + ylim(c(0, 4*10^8)) + ggtitle("Prediction of inpatient visits")
demand_est_yr[which.max(demand_est_yr1$in_p_est),]
p = ggplot(demand_est_yr_age, aes(x = year, y = in_p_est, fill = age)) + geom_area(position = 'stack')+ ggtitle("Prediction of inpatient visits")
ggsave("demand_outp_age.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)
# ggplot(demand_est_yr_age, aes(x = year, y = in_p_est, fill = age)) + geom_area(position = 'fill')+ ggtitle("Prediction of inpatient visits")

demand_est_yr_age = demand_est_yr_age %>% mutate(total_est = out_p_est + in_p_est*3)
p = ggplot(demand_est_yr_age, aes(x = year, y = total_est, fill = age))+ geom_area(position = 'stack') +ylim(c(0, 2*10^9))+ ggtitle(" Prediction of total visits (outpatient + 3*inpatient)")
ggsave("demand_total_age.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)
demand_est_yr1[which.max(demand_est_yr1$total_demand),]
...
start_yr = 2025
result_0<- sup_demand(yr_candi[1], 80, 0, demand_est_yr)
result_250<- sup_demand(yr_candi[2], 80, 0, demand_est_yr)
result_500<- sup_demand(yr_candi[3], 80, 0, demand_est_yr)
result_750<- sup_demand(yr_candi[4], 80, 0, demand_est_yr)
result_1000<- sup_demand(yr_candi[5], 80, 0, demand_est_yr)

## colnames changing
colnames(result_0)[-c(1:3)] <-paste0(colnames(result_0)[-c(1:3)], "_0" )
colnames(result_250)[-c(1:3)] <-paste0(colnames(result_250)[-c(1:3)], "_250" )
colnames(result_500)[-c(1:3)] <-paste0(colnames(result_500)[-c(1:3)], "_500" )
colnames(result_750)[-c(1:3)] <-paste0(colnames(result_750)[-c(1:3)], "_750" )
colnames(result_1000)[-c(1:3)] <-paste0(colnames(result_1000)[-c(1:3)], "_1000" )

result_wo <- result_0 %>% left_join(result_250, by = "year") %>%
  left_join(result_500, by = "year") %>% 
  left_join(result_750, by = "year") %>%
  left_join(result_1000, by = "year")

# Define green gradient colors from light to dark
gradient_colors <- c("black", "lightgreen", "mediumseagreen", "seagreen", "forestgreen")

# Plot with color gradient and legend order
p <- ggplot(result_wo, aes(x = year)) +
  geom_line(aes(y = doc_capa_med_0, col = "+0 (3018)")) +
  geom_line(aes(y = doc_capa_med_250, col = "+250 (3268)")) +
  geom_line(aes(y = doc_capa_med_500, col = "+500 (3518)")) +
  geom_line(aes(y = doc_capa_med_750, col = "+750 (3768)")) +
  geom_line(aes(y = doc_capa_med_1000, col = "+1000 (4018)")) +
  scale_color_manual(
  ...
  ggtitle("The visition capacity per doctors")
ggsave("capa_per_doc.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)

# Define gradient colors from a light to dark scheme
gradient_colors <- c("black", "lightblue", "skyblue", "dodgerblue", "blue")

# Plot with color gradient and legend order
p <- ggplot(result_wo, aes(x = year)) +
  geom_line(aes(y = sup_demand_med_0, col = "+0 (3018)")) +
  geom_line(aes(y = sup_demand_med_250, col = "+250 (3268)")) +
  geom_line(aes(y = sup_demand_med_500, col = "+500 (3518)")) +
  geom_line(aes(y = sup_demand_med_750, col = "+750 (3768)")) +
  geom_line(aes(y = sup_demand_med_1000, col = "+1000 (4018)")) +
  scale_color_manual(
    values = setNames(colorRampPalette(gradient_colors)(5), 
                      c("+0 (3018)", "+250 (3268)", "+500 (3518)", "+750 (3768)", "+1000 (4018)")),
    breaks = c("+0 (3018)", "+250 (3268)", "+500 (3518)", "+750 (3768)", "+1000 (4018)")
  ...
  labs(color = "Supply Levels") +  # Custom legend title
  ggtitle("The difference between supply and demand (without policy duration)")
ggsave("sup_dem_wo.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)

# Policy duration different -----------------------------------------------

start_yr = 2025
result_0_dec<- sup_demand(yr_candi[1], 80, 0, demand_est_yr, start_yr+5, 0.97, tech_rate = 0)
result_250_dec<- sup_demand(yr_candi[2], 80, 0, demand_est_yr, start_yr+5, 0.97, tech_rate = 0)
result_500_dec<- sup_demand(yr_candi[3], 80, 0, demand_est_yr, start_yr+5, 0.97, tech_rate = 0)
result_750_dec<- sup_demand(yr_candi[4], 80, 0, demand_est_yr, start_yr+5, 0.97, tech_rate = 0)
result_1000_dec<- sup_demand(yr_candi[5], 80, 0, demand_est_yr, start_yr+5, 0.97, tech_rate = 0)

## colnames changing
colnames(result_0_dec)[-c(1:3)] <-paste0(colnames(result_0_dec)[-c(1:3)], "_0" )
colnames(result_250_dec)[-c(1:3)] <-paste0(colnames(result_250_dec)[-c(1:3)], "_250" )
colnames(result_500_dec)[-c(1:3)] <-paste0(colnames(result_500_dec)[-c(1:3)], "_500" )
colnames(result_750_dec)[-c(1:3)] <-paste0(colnames(result_750_dec)[-c(1:3)], "_750" )
colnames(result_1000_dec)[-c(1:3)] <-paste0(colnames(result_1000_dec)[-c(1:3)], "_1000" )

result_dec_5y <- result_0_dec %>% left_join(result_250_dec, by = "year") %>%
  left_join(result_500_dec, by = "year") %>% 
  left_join(result_750_dec, by = "year") %>%
  left_join(result_1000_dec, by = "year")

result_0_dec<- sup_demand(yr_candi[1], 80, 0, demand_est_yr, start_yr+10, 0.97, tech_rate = 0)
result_250_dec<- sup_demand(yr_candi[2], 80, 0, demand_est_yr, start_yr+10, 0.97, tech_rate = 0)
result_500_dec<- sup_demand(yr_candi[3], 80, 0, demand_est_yr, start_yr+10, 0.97, tech_rate = 0)
result_750_dec<- sup_demand(yr_candi[4], 80, 0, demand_est_yr, start_yr+10, 0.97, tech_rate = 0)
result_1000_dec<- sup_demand(yr_candi[5], 80, 0, demand_est_yr, start_yr+10, 0.97, tech_rate = 0)
...
result_dec_10y <- result_0_dec %>% left_join(result_250_dec, by = "year") %>%
  left_join(result_500_dec, by = "year") %>% 
  left_join(result_750_dec, by = "year") %>%
  left_join(result_1000_dec, by = "year")

result_0_dec<- sup_demand(yr_candi[1], 80, 0, demand_est_yr, start_yr+15, 0.97, tech_rate = 0)
result_250_dec<- sup_demand(yr_candi[2], 80, 0, demand_est_yr, start_yr+15, 0.97, tech_rate = 0)
result_500_dec<- sup_demand(yr_candi[3], 80, 0, demand_est_yr, start_yr+15, 0.97, tech_rate = 0)
result_750_dec<- sup_demand(yr_candi[4], 80, 0, demand_est_yr, start_yr+15, 0.97, tech_rate = 0)
result_1000_dec<- sup_demand(yr_candi[5], 80, 0, demand_est_yr, start_yr+15, 0.97, tech_rate = 0)
...
result_dec_15y <- result_0_dec %>% left_join(result_250_dec, by = "year") %>%
  left_join(result_500_dec, by = "year") %>% 
  left_join(result_750_dec, by = "year") %>%
  left_join(result_1000_dec, by = "year")

p <- ggplot(result_dec_5y, aes(x = year))+
  geom_line(aes(y = sup_demand_med_tech_0, col = "+0 (3018)")) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed", color = "darkgray") +
  labs(color = "Supply Levels") +  # Custom legend title 
  ggtitle("5 years and 3% decreasing") + ylim(c(-60000, 40000))
ggsave("sup_dem_5yr_0tech.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)

result_dec_5y$year[which(result_dec_5y$sup_demand_med_tech_0 < 0)]
result_dec_5y$sup_demand_med_tech_0[which.min(result_dec_5y$sup_demand_med_tech_0)]
result_dec_5y$year[which.min(result_dec_5y$sup_demand_med_tech_0)]
result_dec_5y$sup_demand_med_tech_0[which(result_dec_5y$year %in% c(2035, 2050))]
result_dec_5y$sup_demand_med_tech_250[which(result_dec_5y$year %in% c(2035, 2050))]
result_dec_5y$sup_demand_med_tech_500[which(result_dec_5y$year %in% c(2035, 2050))]
result_dec_5y$sup_demand_med_tech_750[which(result_dec_5y$year %in% c(2035, 2050))]
result_dec_5y$sup_demand_med_tech_1000[which(result_dec_5y$year %in% c(2035, 2050))]

p <- ggplot(result_dec_10y, aes(x = year))+geom_line(aes(y = sup_demand_med_tech_0, col = "+0 (3018)")) +
  geom_line(aes(y = sup_demand_med_tech_250, col = "+250 (3268)")) + 
  geom_line(aes(y = sup_demand_med_tech_500, col = "+500 (3518)")) +
  geom_line(aes(y = sup_demand_med_tech_750, col = "+750 (3768)")) +
  geom_line(aes(y = sup_demand_med_tech_1000, col = "+1000 (4018)")) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed", color = "darkgray") +
  labs(color = "Supply Levels") +  # Custom legend title 
  ggtitle("10 years and 3% decreasing") + ylim(c(-60000, 40000))
ggsave("sup_dem_10yr_0tech.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)

result_dec_10y$year[which(result_dec_10y$sup_demand_med_tech_0 < 0)]
result_dec_10y$sup_demand_med_tech_0[which.min(result_dec_10y$sup_demand_med_tech_0)]
result_dec_10y$year[which.min(result_dec_10y$sup_demand_med_tech_0)]
result_dec_10y$sup_demand_med_tech_0[which(result_dec_10y$year %in% c(2035, 2050))]
result_dec_10y$sup_demand_med_tech_250[which(result_dec_10y$year %in% c(2035, 2050))]
result_dec_10y$sup_demand_med_tech_500[which(result_dec_10y$year %in% c(2035, 2050))]
result_dec_10y$sup_demand_med_tech_750[which(result_dec_10y$year %in% c(2035, 2050))]
result_dec_10y$sup_demand_med_tech_1000[which(result_dec_10y$year %in% c(2035, 2050))]


p <- ggplot(result_dec_15y, aes(x = year))+geom_line(aes(y = sup_demand_med_tech_0, col = "+0 (3018)")) +
  geom_line(aes(y = sup_demand_med_tech_250, col = "+250 (3268)")) + 
  geom_line(aes(y = sup_demand_med_tech_500, col = "+500 (3518)")) +
  geom_line(aes(y = sup_demand_med_tech_750, col = "+750 (3768)")) +
  geom_line(aes(y = sup_demand_med_tech_1000, col = "+1000 (4018)")) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed", color = "darkgray") +
  labs(color = "Supply Levels") +  # Custom legend title 
  ggtitle("15 years and 3% decreasing") + ylim(c(-60000, 40000))
ggsave("sup_dem_15yr_0tech.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)

result_dec_15y$year[which(result_dec_15y$sup_demand_med_tech_0 < 0)]
result_dec_15y$sup_demand_med_tech_0[which.min(result_dec_15y$sup_demand_med_tech_0)]
result_dec_15y$year[which.min(result_dec_15y$sup_demand_med_tech_0)]
result_dec_15y$sup_demand_med_tech_0[which(result_dec_15y$year %in% c(2035, 2050))]
result_dec_15y$sup_demand_med_tech_250[which(result_dec_15y$year %in% c(2035, 2050))]
result_dec_15y$sup_demand_med_tech_500[which(result_dec_15y$year %in% c(2035, 2050))]
result_dec_15y$sup_demand_med_tech_750[which(result_dec_15y$year %in% c(2035, 2050))]
result_dec_15y$sup_demand_med_tech_1000[which(result_dec_15y$year %in% c(2035, 2050))]

# Policy duration different 0.25% -----------------------------------------------
# With reduction ----------------------------------------------------------

start_yr = 2025
result_0_dec<- sup_demand(yr_candi[1], 80, 0, demand_est_yr, start_yr+5, 0.97, tech_rate = 0.0025)
result_250_dec<- sup_demand(yr_candi[2], 80, 0, demand_est_yr, start_yr+5, 0.97, tech_rate = 0.0025)
result_500_dec<- sup_demand(yr_candi[3], 80, 0, demand_est_yr, start_yr+5, 0.97, tech_rate = 0.0025)
result_750_dec<- sup_demand(yr_candi[4], 80, 0, demand_est_yr, start_yr+5, 0.97, tech_rate = 0.0025)
result_1000_dec<- sup_demand(yr_candi[5], 80, 0, demand_est_yr, start_yr+5, 0.97, tech_rate = 0.0025)
...
result_0_dec<- sup_demand(yr_candi[1], 80, 0, demand_est_yr, start_yr+10, 0.97, tech_rate = 0.0025)
result_250_dec<- sup_demand(yr_candi[2], 80, 0, demand_est_yr, start_yr+10, 0.97, tech_rate = 0.0025)
result_500_dec<- sup_demand(yr_candi[3], 80, 0, demand_est_yr, start_yr+10, 0.97, tech_rate = 0.0025)
result_750_dec<- sup_demand(yr_candi[4], 80, 0, demand_est_yr, start_yr+10, 0.97, tech_rate = 0.0025)
result_1000_dec<- sup_demand(yr_candi[5], 80, 0, demand_est_yr, start_yr+10, 0.97, tech_rate = 0.0025)
...
result_0_dec<- sup_demand(yr_candi[1], 80, 0, demand_est_yr, start_yr+15, 0.97, tech_rate = 0.0025)
result_250_dec<- sup_demand(yr_candi[2], 80, 0, demand_est_yr, start_yr+15, 0.97, tech_rate = 0.0025)
result_500_dec<- sup_demand(yr_candi[3], 80, 0, demand_est_yr, start_yr+15, 0.97, tech_rate = 0.0025)
result_750_dec<- sup_demand(yr_candi[4], 80, 0, demand_est_yr, start_yr+15, 0.97, tech_rate = 0.0025)
result_1000_dec<- sup_demand(yr_candi[5], 80, 0, demand_est_yr, start_yr+15, 0.97, tech_rate = 0.0025)
...
p <- ggplot(result_dec_5y, aes(x = year))+geom_line(aes(y = sup_demand_med_tech_0, col = "+0 (3018)")) +
  geom_line(aes(y = sup_demand_med_tech_250, col = "+250 (3268)")) + 
  geom_line(aes(y = sup_demand_med_tech_500, col = "+500 (3518)")) +
  geom_line(aes(y = sup_demand_med_tech_750, col = "+750 (3768)")) +
  geom_line(aes(y = sup_demand_med_tech_1000, col = "+1000 (4018)")) +
...
ggsave("sup_dem_5yr_025tech.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)
...
ggsave("sup_dem_10yr_025tech.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)
...
ggsave("sup_dem_15yr_025tech.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)
...
# Policy duration different 0.5% -----------------------------------------------
# With reduction ----------------------------------------------------------

start_yr = 2025
result_0_dec<- sup_demand(yr_candi[1], 80, 0, demand_est_yr, start_yr+5, 0.97)
result_250_dec<- sup_demand(yr_candi[2], 80, 0, demand_est_yr, start_yr+5, 0.97)
result_500_dec<- sup_demand(yr_candi[3], 80, 0, demand_est_yr, start_yr+5, 0.97)
result_750_dec<- sup_demand(yr_candi[4], 80, 0, demand_est_yr, start_yr+5, 0.97)
result_1000_dec<- sup_demand(yr_candi[5], 80, 0, demand_est_yr, start_yr+5, 0.97)
...
result_0_dec<- sup_demand(yr_candi[1], 80, 0, demand_est_yr, start_yr+10, 0.97)
result_250_dec<- sup_demand(yr_candi[2], 80, 0, demand_est_yr, start_yr+10, 0.97)
result_500_dec<- sup_demand(yr_candi[3], 80, 0, demand_est_yr, start_yr+10, 0.97)
result_750_dec<- sup_demand(yr_candi[4], 80, 0, demand_est_yr, start_yr+10, 0.97)
result_1000_dec<- sup_demand(yr_candi[5], 80, 0, demand_est_yr, start_yr+10, 0.97)
...
result_0_dec<- sup_demand(yr_candi[1], 80, 0, demand_est_yr, start_yr+15, 0.97)
result_250_dec<- sup_demand(yr_candi[2], 80, 0, demand_est_yr, start_yr+15, 0.97)
result_500_dec<- sup_demand(yr_candi[3], 80, 0, demand_est_yr, start_yr+15, 0.97)
result_750_dec<- sup_demand(yr_candi[4], 80, 0, demand_est_yr, start_yr+15, 0.97)
result_1000_dec<- sup_demand(yr_candi[5], 80, 0, demand_est_yr, start_yr+15, 0.97)
...
ggsave("sup_dem_5yr_05tech.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)
...
ggsave("sup_dem_10yr_05tech.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)
...
ggsave("sup_dem_15yr_05tech.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)
