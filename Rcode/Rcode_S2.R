...
# 시나리오2 -------------------------------------------------------------------
## 2025년부터 2025년까지 out_p_est, in_p_est가 20%씩 감소하는데, 
## 전체 population에 2025년 10%, 2026년 20%, 2027년 30%, 2028년 40%, 2029년이후 50%가 20% 감소.
sup_demand_s2 <- function(admin_set_1, admin_set_2, admin_set_3, demand_est_yr, mid_yr = NULL, reduce_rate, tech_rate = 0.005){
  med_admin_p_1 = c(2533, 3045, 3118, 3309, 3236, 2978, 3018, 3018, 3018) # 의대생: 20대로 가정 (2022년 보다 6년 전 데이터 (2016년) 부터 시작)
  med_admin_p_2 = c(1242, 218, 198, 198, 149, 129, 129, 80, 80) # 의전원: 30대로 가정
  med_admin_p_3 = c(100, 100, 100, 100, 100, 67, 85, 89, 85) # 편입생: 30세로 가정 -> 김영편입학원에서 데이터 가지고 옴 (의대 + 치의대)
  # admin_set_1 = 4977
  ...
  # new_med_total은 60대는 75% 그리고 70대 이상은 50% 정도만 일을 한다고 가정.
  result_sup = result_sup %>% mutate(new_med_total = age20_med + age30_med + age40_med + age50_med + age60_med*0.75 + age70_med*0.5)
  result_sup = result_sup %>% mutate(new_med_8_s2 = case_when(year < 2025 ~ 0,
                                                              year == 2025 ~ round(new_med_total*0.1),
                                                              year == 2026 ~ round(new_med_total*0.2),
                                                              year == 2027 ~ round(new_med_total*0.3),
                                                              year == 2028 ~ round(new_med_total*0.4),
                                                              year >= 2029 ~ round(new_med_total*0.5)),
                                     # new_med_total_s2 = (new_med_total - new_med_8_s2) + new_med_8_s2*0.8
  )
  result <- demand_est_yr %>% left_join(result_sup) %>% dplyr::select(year, out_p_est, in_p_est, license_total, med_total, new_med_total)
  ...
  result <- result %>% mutate(doc_capa = (out_p_est+in_p_est*3)/265/license_total,
                              ...
                              doc_demand_med = (out_p_est+in_p_est*3)/265/51,
                              sup_demand_med = new_med_total - doc_demand_med,
                              out_p_est_s2 = case_when(year < 2025 ~ out_p_est,
                                                       year == 2025 ~ out_p_est*(1-0.1*0.2),
                                                       year == 2026 ~ out_p_est*(1-0.2*0.2),
                                                       year == 2027 ~ out_p_est*(1-0.3*0.2),
                                                       year == 2028 ~ out_p_est*(1-0.4*0.2),
                                                       year >= 2029 ~ out_p_est*(1-0.5*0.2)),
                              in_p_est_s2 = case_when(year < 2025 ~ in_p_est,
                                                      year == 2025 ~ in_p_est*(1-0.1*0.2),
                                                      year == 2026 ~ in_p_est*(1-0.2*0.2),
                                                      year == 2027 ~ in_p_est*(1-0.3*0.2),
                                                      year == 2028 ~ in_p_est*(1-0.4*0.2),
                                                      year >= 2029 ~ in_p_est*(1-0.5*0.2)),
                              # doc_capa_med_s2 = (out_p_est_s2+in_p_est_s2*3)/265/new_med_total_s2,
                              doc_demand_med_s2 = (out_p_est_s2+in_p_est_s2*3)/265/51,
                              # sup_demand_med_s2 = new_med_total_s2 - doc_demand_med_s2,
                              doc_demand_med_tech_s2 = (out_p_est_s2+in_p_est_s2*3)/265/seq(51, by = 51*tech_rate, length.out = (2072 - 2022 + 1)),
                              sup_demand_med_tech_s2 = new_med_total - doc_demand_med_tech_s2
  )
...
result_0_s2<- sup_demand_s2(yr_candi[1], 80, 0, demand_est_yr, mid_yr = start_yr+5, reduce_rate = 0.97, tech_rate = 0.0025)
result_250_s2<- sup_demand_s2(yr_candi[2], 80, 0, demand_est_yr, mid_yr = start_yr+5, reduce_rate = 0.97, tech_rate = 0.0025)
result_500_s2<- sup_demand_s2(yr_candi[3], 80, 0, demand_est_yr, mid_yr = start_yr+5, reduce_rate = 0.97, tech_rate = 0.0025)
result_750_s2<- sup_demand_s2(yr_candi[4], 80, 0, demand_est_yr, mid_yr = start_yr+5, reduce_rate = 0.97, tech_rate = 0.0025)
result_1000_s2<- sup_demand_s2(yr_candi[5], 80, 0, demand_est_yr, mid_yr = start_yr+5, reduce_rate = 0.97, tech_rate = 0.0025)

## colnames changing
colnames(result_0_s2)[-c(1:3)] <-paste0(colnames(result_0_s2)[-c(1:3)], "_0" )
colnames(result_250_s2)[-c(1:3)] <-paste0(colnames(result_250_s2)[-c(1:3)], "_250" )
colnames(result_500_s2)[-c(1:3)] <-paste0(colnames(result_500_s2)[-c(1:3)], "_500" )
colnames(result_750_s2)[-c(1:3)] <-paste0(colnames(result_750_s2)[-c(1:3)], "_750" )
colnames(result_1000_s2)[-c(1:3)] <-paste0(colnames(result_1000_s2)[-c(1:3)], "_1000" )

result_s2_5y <- result_0_s2 %>% left_join(result_250_s2, by = "year") %>%
  left_join(result_500_s2, by = "year") %>% 
  left_join(result_750_s2, by = "year") %>%
  left_join(result_1000_s2, by = "year")

result_0_s2<- sup_demand_s2(yr_candi[1], 80, 0, demand_est_yr, mid_yr = start_yr+10, reduce_rate = 0.97, tech_rate = 0.0025)
result_250_s2<- sup_demand_s2(yr_candi[2], 80, 0, demand_est_yr, mid_yr = start_yr+10, reduce_rate = 0.97, tech_rate = 0.0025)
result_500_s2<- sup_demand_s2(yr_candi[3], 80, 0, demand_est_yr, mid_yr = start_yr+10, reduce_rate = 0.97, tech_rate = 0.0025)
result_750_s2<- sup_demand_s2(yr_candi[4], 80, 0, demand_est_yr, mid_yr = start_yr+10, reduce_rate = 0.97, tech_rate = 0.0025)
result_1000_s2<- sup_demand_s2(yr_candi[5], 80, 0, demand_est_yr, mid_yr = start_yr+10, reduce_rate = 0.97, tech_rate = 0.0025)
...
result_s2_10y <- result_0_s2 %>% left_join(result_250_s2, by = "year") %>%
  left_join(result_500_s2, by = "year") %>% 
  left_join(result_750_s2, by = "year") %>%
  left_join(result_1000_s2, by = "year")

result_0_s2<- sup_demand_s2(yr_candi[1], 80, 0, demand_est_yr, mid_yr = start_yr+15, reduce_rate = 0.97, tech_rate = 0.0025)
result_250_s2<- sup_demand_s2(yr_candi[2], 80, 0, demand_est_yr, mid_yr = start_yr+15, reduce_rate = 0.97, tech_rate = 0.0025)
result_500_s2<- sup_demand_s2(yr_candi[3], 80, 0, demand_est_yr, mid_yr = start_yr+15, reduce_rate = 0.97, tech_rate = 0.0025)
result_750_s2<- sup_demand_s2(yr_candi[4], 80, 0, demand_est_yr, mid_yr = start_yr+15, reduce_rate = 0.97, tech_rate = 0.0025)
result_1000_s2<- sup_demand_s2(yr_candi[5], 80, 0, demand_est_yr, mid_yr = start_yr+15, reduce_rate = 0.97, tech_rate = 0.0025)
...
result_s2_15y <- result_0_s2 %>% left_join(result_250_s2, by = "year") %>%
  left_join(result_500_s2, by = "year") %>% 
  left_join(result_750_s2, by = "year") %>%
  left_join(result_1000_s2, by = "year")

p <- ggplot(result_s2_5y, aes(x = year))+geom_line(aes(y = sup_demand_med_tech_s2_0, col = "+0 (3018)")) +
  geom_line(aes(y = sup_demand_med_tech_s2_250, col = "+250 (3268)")) + 
  geom_line(aes(y = sup_demand_med_tech_s2_500, col = "+500 (3518)")) +
  geom_line(aes(y = sup_demand_med_tech_s2_750, col = "+750 (3768)")) +
  geom_line(aes(y = sup_demand_med_tech_s2_1000, col = "+1000 (4018)")) +
  ylab("") + 
  scale_color_manual(
...
ggsave("sup_dem_5yr_025tech_s2.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)

result_s2_5y$year[which(result_s2_5y$sup_demand_med_tech_s2_0 < 0)]
result_s2_5y$sup_demand_med_tech_s2_0[which.min(result_s2_5y$sup_demand_med_tech_s2_0)]
result_s2_5y$year[which.min(result_s2_5y$sup_demand_med_tech_s2_0)]

p <- ggplot(result_s2_10y, aes(x = year))+geom_line(aes(y = sup_demand_med_tech_s2_0, col = "+0 (3018)")) +
  geom_line(aes(y = sup_demand_med_tech_s2_250, col = "+250 (3268)")) + 
  geom_line(aes(y = sup_demand_med_tech_s2_500, col = "+500 (3518)")) +
  geom_line(aes(y = sup_demand_med_tech_s2_750, col = "+750 (3768)")) +
  geom_line(aes(y = sup_demand_med_tech_s2_1000, col = "+1000 (4018)")) +
...
ggsave("sup_dem_10yr_025tech_s2.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)

result_s2_10y$year[which(result_s2_10y$sup_demand_med_tech_s2_0 < 0)]
result_s2_10y$sup_demand_med_tech_s2_0[which.min(result_s2_10y$sup_demand_med_tech_s2_0)]
result_s2_10y$year[which.min(result_s2_10y$sup_demand_med_tech_s2_0)]

p <- ggplot(result_s2_15y, aes(x = year))+geom_line(aes(y = sup_demand_med_tech_s2_0, col = "+0 (3018)")) +
  geom_line(aes(y = sup_demand_med_tech_s2_250, col = "+250 (3268)")) + 
  geom_line(aes(y = sup_demand_med_tech_s2_500, col = "+500 (3518)")) +
  geom_line(aes(y = sup_demand_med_tech_s2_750, col = "+750 (3768)")) +
  geom_line(aes(y = sup_demand_med_tech_s2_1000, col = "+1000 (4018)")) +
...
ggsave("sup_dem_15yr_025tech_s2.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)

result_s2_15y$year[which(result_s2_15y$sup_demand_med_tech_s2_0 < 0)]
result_s2_15y$sup_demand_med_tech_s2_0[which.min(result_s2_15y$sup_demand_med_tech_s2_0)]
result_s2_15y$year[which.min(result_s2_15y$sup_demand_med_tech_s2_0)]


# S2: tech_rate: 0.5% -----------------------------------------------------

result_0_s2<- sup_demand_s2(yr_candi[1], 80, 0, demand_est_yr, mid_yr = start_yr+5, reduce_rate = 0.97)
result_250_s2<- sup_demand_s2(yr_candi[2], 80, 0, demand_est_yr, mid_yr = start_yr+5, reduce_rate = 0.97)
result_500_s2<- sup_demand_s2(yr_candi[3], 80, 0, demand_est_yr, mid_yr = start_yr+5, reduce_rate = 0.97)
result_750_s2<- sup_demand_s2(yr_candi[4], 80, 0, demand_est_yr, mid_yr = start_yr+5, reduce_rate = 0.97)
result_1000_s2<- sup_demand_s2(yr_candi[5], 80, 0, demand_est_yr, mid_yr = start_yr+5, reduce_rate = 0.97)
...
result_0_s2<- sup_demand_s2(yr_candi[1], 80, 0, demand_est_yr, mid_yr = start_yr+10, reduce_rate = 0.97)
result_250_s2<- sup_demand_s2(yr_candi[2], 80, 0, demand_est_yr, mid_yr = start_yr+10, reduce_rate = 0.97)
result_500_s2<- sup_demand_s2(yr_candi[3], 80, 0, demand_est_yr, mid_yr = start_yr+10, reduce_rate = 0.97)
result_750_s2<- sup_demand_s2(yr_candi[4], 80, 0, demand_est_yr, mid_yr = start_yr+10, reduce_rate = 0.97)
result_1000_s2<- sup_demand_s2(yr_candi[5], 80, 0, demand_est_yr, mid_yr = start_yr+10, reduce_rate = 0.97)
...
result_0_s2<- sup_demand_s2(yr_candi[1], 80, 0, demand_est_yr, mid_yr = start_yr+15, reduce_rate = 0.97)
result_250_s2<- sup_demand_s2(yr_candi[2], 80, 0, demand_est_yr, mid_yr = start_yr+15, reduce_rate = 0.97)
result_500_s2<- sup_demand_s2(yr_candi[3], 80, 0, demand_est_yr, mid_yr = start_yr+15, reduce_rate = 0.97)
result_750_s2<- sup_demand_s2(yr_candi[4], 80, 0, demand_est_yr, mid_yr = start_yr+15, reduce_rate = 0.97)
result_1000_s2<- sup_demand_s2(yr_candi[5], 80, 0, demand_est_yr, mid_yr = start_yr+15, reduce_rate = 0.97)
...
ggsave("sup_dem_5yr_05tech_s2.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)
...
ggsave("sup_dem_10yr_05tech_s2.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)
...
ggsave("sup_dem_15yr_05tech_s2.png", plot = p, path = plot_dir, width = 8, height = 5, dpi = 600)
